...
`CloudRuntime` keeps the runtime information of the server and workers. `CloudProvider` provides the functions to run the container on the cloud and `DockerContainer` defines which container should be used. As the cluster might need to deploy both the server and workers, the `DockerCluster` object defines both `serverContainer` and `workerContainer` as its slots. Since a `DockerCluster` object corresponds to a cluster on the cloud, all of the beforementioned components behave like the environment object in R. Therefore, it is possible to change the value in the `DockerCluster` object inside a function and broadcast the effect to the same object outside of the function.
...
## Accessor functions
The package provides getters/setters for the developer to access the values in the `DockerCluster` object. The high-level accessors are
```{r eval=FALSE}
.getCloudProvider
.getCloudConfig
.getServerContainer
...
.getCloudRuntime
```
Note that there are no setters for them as they are of the reference class. The accessors for the `CloudConfig` are
```{r eval=FALSE}
## Getter
.getJobQueueName
.getWorkerNumber
...
.setServerPort
```
The accessors for `CloudRuntime` are
```{r eval=FALSE}
## Getter
.getWorkerHandles
.getServerHandle
...
.setServerPublicIp
```
In most cases, only the getters are required to be called in the `CloudProvider` or `DockerContainer` functions.
...
```
where `name` is the name of the container, `maxWorkerNum` defines the maximum number of workers that can be run in the same container. `environment` is the environment variable in the container. `image` is the image used by the container. `sysPackages` is a vector of the system packages that need to be installed before the R worker starts(e.g. packages installed by `apt-get install`). `RPackages` is a vector of R packages that should be installed in the container. Note that a minimum container should define at least the `image` slot. The other slots are optional.
...
## Parallel backend
It is the container's duty to register the parallel backend as neither the cloud provider nor the cluster knows which backend your container uses. The backend can be anything defined globally that the user could use to do the parallel computing. The popular choices are `foreach` or `BiocParallel`. Other backends are also possible. Note that if your backend is not from `foreach`, you should also define `deregisterParallelBackend` as the default method deregisters the foreach backend.

## Server container
The function `getServerContainer` is purely for obtaining the server container from the worker container. By doing so the user only need to provide the worker container to the cluster. This function is optional, but we recommend to define it as if no method is defined, the user must explicitly provide both the server and worker container to the cluster if both need to be run on the cloud.

## Extension to the container
You can export the functions and variables in the container via `getExportedNames` and `getExportedObject`. 
`getExportedNames` defines the exported names and `getExportedObject` gets the exported object.
They will be called by the cluster and the user can see the exported objects from `DockerCluster$workerContainer$...`. Note that `DockerCluster` has defined these two functions as well, if you need to override them, you may need to call `callNextMethod` to get the results from the parent function.
...
```
where `container` should be an object that has `DockerContainer` as its parent class. `hardware` is from the class `CloudHardware`. `workerNumber` indicates how many workers should be run on the cloud. The return value of the `runDockerServer` should be a server handle and `runDockerWorkers` should be a list of worker handles where each handle corresponds to a worker. The worker handles can be duplicated if multiple workers share the same container. The handle can be any object that is used by the cloud provider to identify the running container.

The cloud provider should respect any settings in the argument `container`. The environment variables in the container has been configured before passing to the cloud provider as [The big picture](#The-big-picture)) shows. However, the worker number is set to 1 for each container. If the cloud provider have enough resources to support multiple workers in one container, the provider should call `configWorkerContainerEnv` with the new worker number again to overwrite the previous settings. It is providers responsibility to make sure the worker number does not exceed the number limitation specified in `DockerContainer$maxWorkerNum`

The argument `hardware` specifies the hardware for each container. As the hardware is different for each cloud provider. The base class `CloudHardware` only contains the most import hardware parameters, that is, it only has the slots `cpu`, `memory` and `id`. Although the cluster does not have any hard restriction on how the provider should explain these three parameters, we recommend to explain them as follow
...
3. `id`: The id of the hardware. This can be missing if the cloud provider do not need this slot.
